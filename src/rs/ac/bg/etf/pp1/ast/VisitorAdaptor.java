// generated with ast extension for cup
// version 0.8
// 27/1/2023 18:53:9


package rs.ac.bg.etf.pp1.ast;

public abstract class VisitorAdaptor implements Visitor { 

    public void visit(MethodDecl MethodDecl) { }
    public void visit(MatchedStatement MatchedStatement) { }
    public void visit(DesignatorStatementUnpackPart DesignatorStatementUnpackPart) { }
    public void visit(Var Var) { }
    public void visit(StatementList StatementList) { }
    public void visit(ConditionTerm ConditionTerm) { }
    public void visit(DesignatorStatementUnpackList DesignatorStatementUnpackList) { }
    public void visit(Factor Factor) { }
    public void visit(VarList VarList) { }
    public void visit(Designator Designator) { }
    public void visit(FormalParList FormalParList) { }
    public void visit(Term Term) { }
    public void visit(Condition Condition) { }
    public void visit(MulOp MulOp) { }
    public void visit(RelOp RelOp) { }
    public void visit(ConstVar ConstVar) { }
    public void visit(GlobalDeclList GlobalDeclList) { }
    public void visit(VarDeclList VarDeclList) { }
    public void visit(ConstVal ConstVal) { }
    public void visit(Expr Expr) { }
    public void visit(FormalPar FormalPar) { }
    public void visit(AddOp AddOp) { }
    public void visit(DesignatorStatement DesignatorStatement) { }
    public void visit(FormalPars FormalPars) { }
    public void visit(ActualPars ActualPars) { }
    public void visit(UnmatchedStatement UnmatchedStatement) { }
    public void visit(BoolConstVal BoolConstVal) { }
    public void visit(BaseExpr BaseExpr) { }
    public void visit(Statement Statement) { }
    public void visit(ReturnExpr ReturnExpr) { }
    public void visit(ConstVarList ConstVarList) { }
    public void visit(ClassDecl ClassDecl) { }
    public void visit(ConstDecl ConstDecl) { }
    public void visit(MethodDeclList MethodDeclList) { }
    public void visit(ConditionFactor ConditionFactor) { }
    public void visit(GlobalDecl GlobalDecl) { }
    public void visit(ActualParList ActualParList) { }
    public void visit(PrintExpr PrintExpr) { }
    public void visit(MulOpMod MulOpMod) { visit(); }
    public void visit(MulOpDiv MulOpDiv) { visit(); }
    public void visit(MulOpTimes MulOpTimes) { visit(); }
    public void visit(AddOpMinus AddOpMinus) { visit(); }
    public void visit(AddOpPlus AddOpPlus) { visit(); }
    public void visit(RelOpLTE RelOpLTE) { visit(); }
    public void visit(RelOpLT RelOpLT) { visit(); }
    public void visit(RelOpGTE RelOpGTE) { visit(); }
    public void visit(RelOpGT RelOpGT) { visit(); }
    public void visit(RelOpNotEqual RelOpNotEqual) { visit(); }
    public void visit(RelOpEqual RelOpEqual) { visit(); }
    public void visit(AssignOp AssignOp) { visit(); }
    public void visit(GroupedFactor GroupedFactor) { visit(); }
    public void visit(NewArrayFactor NewArrayFactor) { visit(); }
    public void visit(BoolConstFactor BoolConstFactor) { visit(); }
    public void visit(CharConstFactor CharConstFactor) { visit(); }
    public void visit(NumConstFactor NumConstFactor) { visit(); }
    public void visit(FuncCallFactor FuncCallFactor) { visit(); }
    public void visit(DesignatorFactor DesignatorFactor) { visit(); }
    public void visit(SingleTerm SingleTerm) { visit(); }
    public void visit(MulOpTerm MulOpTerm) { visit(); }
    public void visit(SingleTermExpression SingleTermExpression) { visit(); }
    public void visit(AddOpExpression AddOpExpression) { visit(); }
    public void visit(NegatedExpression NegatedExpression) { visit(); }
    public void visit(Expression Expression) { visit(); }
    public void visit(SingleFactor SingleFactor) { visit(); }
    public void visit(MultiFactor MultiFactor) { visit(); }
    public void visit(SingleFactorTermCondition SingleFactorTermCondition) { visit(); }
    public void visit(MultiFactorTermCondition MultiFactorTermCondition) { visit(); }
    public void visit(SingleTermCondition SingleTermCondition) { visit(); }
    public void visit(MultiTermCondition MultiTermCondition) { visit(); }
    public void visit(SingleActualParam SingleActualParam) { visit(); }
    public void visit(MultiActualParam MultiActualParam) { visit(); }
    public void visit(NoActualParamList NoActualParamList) { visit(); }
    public void visit(ActualParamList ActualParamList) { visit(); }
    public void visit(PrintExprNoWidth PrintExprNoWidth) { visit(); }
    public void visit(PrintExprWidth PrintExprWidth) { visit(); }
    public void visit(NoRetExpr NoRetExpr) { visit(); }
    public void visit(RetExpr RetExpr) { visit(); }
    public void visit(ArrayDesignator ArrayDesignator) { visit(); }
    public void visit(VarDesignator VarDesignator) { visit(); }
    public void visit(NoDesignatorPart NoDesignatorPart) { visit(); }
    public void visit(DesignatorPart DesignatorPart) { visit(); }
    public void visit(UnpackSingle UnpackSingle) { visit(); }
    public void visit(UnpackList UnpackList) { visit(); }
    public void visit(UnpackStmt UnpackStmt) { visit(); }
    public void visit(DecStmt DecStmt) { visit(); }
    public void visit(IncStmt IncStmt) { visit(); }
    public void visit(FuncCallStmt FuncCallStmt) { visit(); }
    public void visit(AssignStmt AssignStmt) { visit(); }
    public void visit(MultiStmt MultiStmt) { visit(); }
    public void visit(IfElseStmt IfElseStmt) { visit(); }
    public void visit(ForeachStmt ForeachStmt) { visit(); }
    public void visit(PrintStmt PrintStmt) { visit(); }
    public void visit(ReadStmt ReadStmt) { visit(); }
    public void visit(ReturnStmt ReturnStmt) { visit(); }
    public void visit(ContinueStmt ContinueStmt) { visit(); }
    public void visit(BreakStmt BreakStmt) { visit(); }
    public void visit(WhileStmt WhileStmt) { visit(); }
    public void visit(DesignatorStmt DesignatorStmt) { visit(); }
    public void visit(UnmatchedIfElseStmt UnmatchedIfElseStmt) { visit(); }
    public void visit(UnmatchedIfStmt UnmatchedIfStmt) { visit(); }
    public void visit(UnmatchedStmt UnmatchedStmt) { visit(); }
    public void visit(MatchedStmt MatchedStmt) { visit(); }
    public void visit(NoStmt NoStmt) { visit(); }
    public void visit(StmtList StmtList) { visit(); }
    public void visit(FormalParam FormalParam) { visit(); }
    public void visit(SingleFormalParam SingleFormalParam) { visit(); }
    public void visit(MultiFormalParam MultiFormalParam) { visit(); }
    public void visit(NoFormalParamList NoFormalParamList) { visit(); }
    public void visit(FormalParamList FormalParamList) { visit(); }
    public void visit(VoidMethodDecl VoidMethodDecl) { visit(); }
    public void visit(TypedMethodDecl TypedMethodDecl) { visit(); }
    public void visit(NoVars NoVars) { visit(); }
    public void visit(VarDecls VarDecls) { visit(); }
    public void visit(NoMethods NoMethods) { visit(); }
    public void visit(MethodDecls MethodDecls) { visit(); }
    public void visit(BoolConstFalse BoolConstFalse) { visit(); }
    public void visit(BoolConstTrue BoolConstTrue) { visit(); }
    public void visit(Type Type) { visit(); }
    public void visit(ConstValDerived1 ConstValDerived1) { visit(); }
    public void visit(BoolConst BoolConst) { visit(); }
    public void visit(CharConst CharConst) { visit(); }
    public void visit(NumericConst NumericConst) { visit(); }
    public void visit(ConstVarScalar ConstVarScalar) { visit(); }
    public void visit(SingleConstVar SingleConstVar) { visit(); }
    public void visit(MultiConstVar MultiConstVar) { visit(); }
    public void visit(VarArray VarArray) { visit(); }
    public void visit(VarScalar VarScalar) { visit(); }
    public void visit(SingleVar SingleVar) { visit(); }
    public void visit(MultiVar MultiVar) { visit(); }
    public void visit(ConstVarDecl ConstVarDecl) { visit(); }
    public void visit(VarDecl VarDecl) { visit(); }
    public void visit(ConstructorDecl ConstructorDecl) { visit(); }
    public void visit(ExtendedClassWithMethodsDecl ExtendedClassWithMethodsDecl) { visit(); }
    public void visit(ExtendedClassNoMethodsDecl ExtendedClassNoMethodsDecl) { visit(); }
    public void visit(BaseClassWithMethodsDecl BaseClassWithMethodsDecl) { visit(); }
    public void visit(BaseClassNoMethodsDecl BaseClassNoMethodsDecl) { visit(); }
    public void visit(GlobalClassDecl GlobalClassDecl) { visit(); }
    public void visit(GlobalConstDecl GlobalConstDecl) { visit(); }
    public void visit(GlobalVarDecl GlobalVarDecl) { visit(); }
    public void visit(NoDeclsGlobal NoDeclsGlobal) { visit(); }
    public void visit(GlobalDecls GlobalDecls) { visit(); }
    public void visit(Prog Prog) { visit(); }


    public void visit() { }
}
